Engine.Plugins.b3 = {
	init:function(){
		Engine.GUI.Sidebar.Nav.add('b3', 'main_navigation');
		...
		var checkNav = setInterval(function() {
			...
			if(a.length > 0){
				...
				ul.find('a').off("click");
				ul.find('a[href="?p=init"]').removeAttr('href').off().click(function(){ Engine.init(); });
				ul.find('a[href="?p=test"]').removeAttr('href').off().click(function(){
					Engine.NEWrequest('b3','read',function(dataset){
	...
	load:{
		index:function(){
			Engine.Builder.card($('#pagecontent'),{ title: 'b3', icon: 'b3'}, function(card){
				Engine.request('b3','read',{
					data:{options:{ link_to:'b3Index',plugin:'b3',view:'index' }},
				},function(result) {
					var dataset = JSON.parse(result);
					if(dataset.success != undefined){
						for(const [key, value] of Object.entries(dataset.output.dom)){ Engine.Helper.set(Engine.Contents,['data','dom','b3',value.id],value); }
						for(const [key, value] of Object.entries(dataset.output.raw)){ Engine.Helper.set(Engine.Contents,['data','raw','b3',value.id],value); }
		...
		details:function(){
			...
			Engine.request(url.searchParams.get("p"),'get',{data:{id:id,key:'transaction_number'}},function(result){
				var dataset = JSON.parse(result);
				if(dataset.success != undefined){
					...
					Engine.GUI.Layouts.details.build(dataset.output,container,{title:"B3 Details",image:bgImage},function(data,layout){
						if(layout.main.parents().eq(2).parent('.modal-body').length > 0){
							var modal = layout.main.parents().eq(2).parent('.modal-body').parents().eq(2);
							if(Engine.Auth.validate('plugin', 'b3', 3)){
						...
						Engine.GUI.Layouts.details.tab(data,layout,{icon:"fas fa-history",text:Engine.Contents.Language["History"]},function(data,layout,tab,content){
							Engine.Helper.set(Engine.Contents,['layouts','b3',data.this.raw.id,layout.main.attr('id')],layout);
							...
							// Clear
							if(Engine.Auth.validate('custom', 'b3_clear', 1)){
								Engine.GUI.Layouts.details.control(data,layout,{color:"danger",icon:"fas fa-snowplow",text:Engine.Contents.Language["Clear"]},function(data,layout,button){
									button.off().click(function(){
										Engine.request('b3','clear',{ data:data.this.raw },function(){
											Engine.Plugins.b3.load.details();
										});
									});
								});
							}
							// Review
							if(Engine.Auth.validate('custom', 'b3_review', 1)){
								Engine.GUI.Layouts.details.control(data,layout,{color:"success",icon:"fas fa-search-location",text:Engine.Contents.Language["Review"]},function(data,layout,button){
									button.off().click(function(){
										Engine.Plugins.b3.review(data,layout);
									});
								});
							}
							// Transaction Number
							Engine.GUI.Layouts.details.data(data,layout,{field:"transaction_number"});
							// Billing Invoice Number
							Engine.GUI.Layouts.details.data(data,layout,{field:"invoice_number"});
							// Status
							if(Engine.Helper.isSet(Engine.Plugins,['statuses']) && Engine.Auth.validate('custom', 'b3_statuses', 1)){
								Engine.Plugins.statuses.Layouts.details.detail(data,layout);
							}
							// Organizations
							if(Engine.Helper.isSet(Engine.Plugins,['organizations']) && Engine.Auth.validate('custom', 'b3_organizations', 1)){
								Engine.Plugins.organizations.Layouts.details.detail(data,layout);
							}
							// Client
							Engine.GUI.Layouts.details.data(data,layout,{field:"b3_client_code"});
							// SBRN
							Engine.GUI.Layouts.details.data(data,layout,{field:"client_sbrn"});
							// Cargo Control Number
							Engine.GUI.Layouts.details.data(data,layout,{field:"cargo_control_number"});
							// Customs Office
							Engine.GUI.Layouts.details.data(data,layout,{field:"customs_office"});
							// Sub Location
							Engine.GUI.Layouts.details.data(data,layout,{field:"location_of_goods"});
							// Master WayBill
							Engine.GUI.Layouts.details.data(data,layout,{field:"mwb"});
							// Container Number(s)
							Engine.GUI.Layouts.details.data(data,layout,{field:"container_numbers"});
							// Vendor
							Engine.GUI.Layouts.details.data(data,layout,{field:"vendor_code"});
							// Purchase Order
							Engine.GUI.Layouts.details.data(data,layout,{field:"po_num"});
							// Invoice Number
							Engine.GUI.Layouts.details.data(data,layout,{field:"ci_num"});
							// Number of Packages
							Engine.GUI.Layouts.details.data(data,layout,{field:"num_pkg"});
							// Weight
							Engine.GUI.Layouts.details.data(data,layout,{field:"gross_weight"});
							// Notes
							if(Engine.Helper.isSet(Engine.Plugins,['notes']) && Engine.Auth.validate('custom', 'b3_notes', 1)){
								Engine.Plugins.notes.Layouts.details.tab(data,layout);
							}
							// Contacts
							if(Engine.Helper.isSet(Engine.Plugins,['contacts']) && Engine.Auth.validate('custom', 'b3_contacts', 1)){
							...
							Engine.GUI.Layouts.details.button(data,layout,{icon:icon},function(data,layout,button){
								button.off().click(function(){
									if(button.find('i').hasClass( "fa-bell" )){
										...
										Engine.request("b3",'subscribe',{data:{id:data.this.raw.id}},function(answer){
							...
							// Timeline
							Engine.Builder.Timeline.render(data,layout,{prefix:"b3_"});
						});
					});
				}
			});
	...
	review:function(data,layout,options = {},callback = null){
		...
		var defaults = {icon: Engine.Plugins.b3.Timeline.icon,color: "secondary"};
		...
		Engine.Builder.modal($('body'), {
			...
			if(Engine.Helper.isSet(data,['relations','files'])){
				for(var [id, file] of Object.entries(data.relations.files)){
					if(file.filename.includes("MD"+data.this.raw.transaction_number) || file.filename.includes("DOCSET")){
						files.append(Engine.Plugins.files.Layouts.details.GUI.button(file,{download:Engine.Auth.validate('custom', url.searchParams.get("p")+'_files', 1),download:Engine.Auth.validate('custom', url.searchParams.get("p")+'_files', 4)}));
			...
			controls.append('<button class="btn btn-warning" data-action="corrections"><i class="fas fa-pencil-alt mr-1"></i>'+Engine.Contents.Language['To Correct']+'</button>');
			controls.append('<button class="btn btn-success" data-action="reviewed"><i class="fas fa-check mr-1"></i>'+Engine.Contents.Language['Reviewed']+'</button>');
					...
					if(note != undefined && note != ''){
						Engine.request(url.searchParams.get("p"),'note',{data:note},function(result){
							...
							if(noteData.success != undefined){
								Engine.Plugins.notes.Timeline.object(noteData.output.note.dom,layout);
								data.this.raw.status = 7;
								data.this.dom.status = 7;
								Engine.request('b3','review',{data:data.this.raw},function(result){
	...
	Timeline:{
		...
		object:function(dataset,layout,options = {},callback = null){
			if(options instanceof Function){ callback = options; options = {}; }
			var defaults = {icon: Engine.Plugins.b3.Timeline.icon,color: "indigo"};
			for(var [key, option] of Object.entries(options)){ if(Engine.Helper.isSet(defaults,[key])){ defaults[key] = option; } }
			if(typeof dataset.id !== 'undefined'){
				...
				var checkExist = setInterval(function() {
					if(layout.timeline.find('div.time-label[data-dateus="'+dateUS+'"]').length > 0){
						...
						Engine.Builder.Timeline.add.filter(layout,'b3','B3');
						var html = '';
						...
						element.find('i').first().off().click(function(){
							Engine.CRUD.read.show({ key:'transaction_number',keys:dataset, href:"?p=b3&v=details&id="+dataset.transaction_number, modal:true });
						});
